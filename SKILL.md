---
name: hotspots_xwlb
description: Consumes hotspot content from https://hotspot.api4claw.com/hotspots/platform/新闻联播 and presents it to end users. Response format is JSON array (sources with items). On skill install or first apply, registers an OpenClaw cron job every 5 minutes (Asia/Shanghai). Use when users ask for 新闻联播热点、status、按 source_name 分组标题，或当定时消息触发该 skill。
---
...
openclaw cron add \
  ...
  --to "当前会话的用户ID（自动替换）" \
  --message '定时执行 hotspots_xwlb skill：请读取 skill「hotspots_xwlb」并按 Consumer Workflow 执行「新闻联播热点」——请求 GET https://hotspot.api4claw.com/hotspots/platform/新闻联播；按 JSON 解析并直接展示接口内容（按 source_name 分组标题）。'
```
...
1. 视为 **新闻联播热点**：对 `GET https://hotspot.api4claw.com/hotspots/platform/新闻联播` 请求一次（除非用户只要 status；定时默认输出完整最新内容），并按 JSON 结构处理。
...
- read 新闻联播 hotspot data from `/hotspots/platform/新闻联播` (primary: JSON)
...
- `GET /hotspots/platform/新闻联播`: fetch 新闻联播 hotspot payload as JSON array. Each element is a source block (e.g. `source`, `source_name`, `fetched_at`, `items[]`), and each item usually includes `title`, `content`, `link`, `hotness`.
...
- `新闻联播热点`:
  1. Call `GET /hotspots/platform/新闻联播`.
...
- `status`: call `GET /hotspots/platform/新闻联播`, then report reachable/unreachable and basic stats from JSON (source count, total item count). Do NOT show `fetched_at` or `data_date`.
...
- endpoint used: `/hotspots/platform/新闻联播`
- Do NOT show `fetched_at` or `data_date`

## Reliability Rules

- If `/hotspots/platform/新闻联播` fails, return explicit failure reason.
...
- Do not call any hotspot endpoint except `/hotspots/platform/新闻联播`.
